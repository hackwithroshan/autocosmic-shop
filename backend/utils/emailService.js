...
const triggerEmailAPI = async (payload) => {
    try {
        ...
        if (!frontendUrl) {
            ...
            console.error("👉 Please set FRONTEND_URL = https://your-vercel-app.vercel.app in Railway Settings");
        ...
        // Simple timeout logic (15 seconds to allow cold start)
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error(`❌ Vercel API Failed: ${response.status}`);
                console.error(`   Details: ${errText.substring(0, 200)}`); // Log first 200 chars of error
                return { success: false, error: `Email API returned ${response.status}` };
            }

            const data = await response.json();
            console.log("✅ Email Request Accepted by Vercel");
            return { success: true, data };

        } catch (fetchError) {
            clearTimeout(timeoutId);
            if (fetchError.name === 'AbortError') {
                console.error("❌ Email API Timeout (Vercel function took > 15s to respond)");
                return { success: false, error: "Email service timed out" };
            }
            console.error("❌ Network Error connecting to Vercel:", fetchError.message);
            throw fetchError;
        }
